...
class button:
    ...
    def only():#被按钮调用抽一次的函数
        ...
        for key,value in all_change.items():
            if (value['bg']=='purple'or value['bg']=='red') and str(key+1) in all_label:
        ...
        for i in range(int(first_girl.get()),int(class_size.get())):
            ...
            if EXCLUDE_GIRL.get():
                if all_change[i]['bg'] != 'purple':
                    all_change[i].config(bg='red')
                if str(all_change[i]['text']) in all_label:
                    all_label.remove(str(all_change[i]['text']))
            if i-1 in all_change and all_change[i-1]['bg']=='#C0C0C0' and str(i) not in all_label and all_change[i-1]!='purple':
        ...
        if EXCLUDE_GIRL.get():
            for i in range(int(first_girl.get()),int(class_size.get())+1):
                if i-1 in all_change and all_change[i-1]['bg']=='#C0C0C0' and str(i) not in all_label:
                    all_label.append(str(i))
                if all_change[i-1] != 'purple':
                    all_change[i-1].config(bg='red')
                if (all_change[i-1]['bg']=='purple' or all_change[i-1]['bg']=='red') and str(i) in all_label:
        ...
        if EXCLUDE_BOY.get():
            for i in range(0,int(first_girl.get())-1):
                ...
                if all_change[i]['bg'] != 'purple':
                    all_change[i].config(bg='red')
                if (all_change[i]['bg']=='red' or all_change[i]['bg']=='purple') and str(i+1)in all_label:
...
def init_start():
    try:
        ...
        raise 〇('若有更多需要请访问项目地址:https://github.com/QO0o/RandomExtraction')
...
class add_del:  
    ...
    def run_add_del(self,ordinal):
        if all_change[ordinal]['bg']=='#C0C0C0':
            ...
            while re_0.get()and re_1.get() and str(ordinal+1) in has_label and not EXCLUDE_BOY.get() and EXCLUDE_GIRL.get() and str(ordinal+1) not in [str(i) for i in list(range(int(first_girl.get()),int(class_size.get())+1))]:
                has_label.remove(str(ordinal+1))
            while re_0.get()and re_1.get() and str(ordinal+1) in has_label and not EXCLUDE_GIRL.get() and EXCLUDE_BOY.get() and str(ordinal+1) not in [str(i) for i in list(range(1,int(first_girl.get())))]:
                has_label.remove(str(ordinal+1))
            while re_0.get()and re_1.get() and str(ordinal+1) in has_label and not EXCLUDE_BOY.get() and not EXCLUDE_GIRL.get():
                has_label.remove(str(ordinal+1))
...
class check:
    ...
    def clear_has():
        ...
        if re_0.get() and re_1.get():
            for NI in deleted:
                if NI not in has_label and NI !='0':
                    has_label.append(NI)
        for key,value in all_change.items():
            if not re_1.get() and value['bg']=='purple':
                value.config(bg='red')
                if key+1 in deleted:
    ...
    def open_all():
        ...
        for key,value in all_change.items():
            ...
            if (not re_1.get() or not re_0.get()) and value['bg']=='purple':
                value.config(bg='red')
            if re_1.get() and str(value['text']) not in deleted and str(value['text'])not in all_label: 
                value.config(bg='purple')
                if key+1 in deleted:
                    deleted.remove(str(key+1))
                if str(key+1) not in all_label:
                    all_label.append(str(key+1))
        ...
        if re_0.get()and re_1.get():
            for m in has_label:
                ...
                if all_change[int(m)-1]['bg']=='#C0C0C0' and all_change[int(m)-1]!='purple':
                    all_change[int(m)-1].config(bg='red')
                if m not in deleted:
                    deleted.append(m)
    ...
    def no_re():
        ...
        if re_1.get()and random_num in all_label:
            ...
            if int(random_num) >= 1:
                if all_change[int(random_num)-1]!='pueple':
...
class exclude:
    ...
    def boy():
        ...
        for B in range(0,int(first_girl.get())-1):
            if EXCLUDE_BOY.get():
                if all_change[B]['bg'] != 'purple':
                    all_change[B].config(bg='red')
                    if str(B+1) in all_label:
                        all_label.remove(str(B+1))
                    if str(B+1) not in has_label:
                        has_label.append(str(B+1)) 
            else:
                if all_change[B]['bg'] != 'purple':
                    all_change[B].config(bg='#C0C0C0')
                    if str(B+1) not in all_label:
                        all_label.append(str(B+1))
                    while all_change[B]['text'] in has_label:
                        has_label.remove(all_change[B]['text'])
    ...
    def girl():
        ...
        for G in range(int(first_girl.get()),int(class_size.get())+1):
            if EXCLUDE_GIRL.get():
                try:
                    if all_change[G-1]['bg'] != 'purple':
                        all_change[G-1].config(bg='red')
                except KeyError as e:
                    print('待初始化')
                if str(G) in all_label:
                    all_label.remove(str(G)) 
                if str(G) not in has_label:
                    has_label.append(str(G))
            else:
                if all_change[G-1]['bg'] != 'purple':
                    all_change[G-1].config(bg='#C0C0C0')
                    if str(G) not in all_label:
                        all_label.append(str(G))
                    while all_change[G-1]['text'] in has_label:
                        has_label.remove(all_change[G-1]['text'])
        len_has.config(text=f"已有数目:{len(set(has_label))}")

def Negate_command():
    for key,value in all_change.items():
        if EXCLUDE_BOY.get() and value['text'] in [str(i) for i in list(range(1,int(first_girl.get())))] and not EXCLUDE_GIRL.get():
            if value['bg']=='red':
        ...
        if EXCLUDE_GIRL.get() and value['text'] in [str(i) for i in list(range(1,int(first_girl.get())))] and not EXCLUDE_BOY.get():
            if value['bg']=='red':
                value.config(bg='#C0C0C0')
                if str(key+1) not in all_label:
                    all_label.append(str(key+1))
                ...
                if str(key+1) in all_label:
                    ...
                    if str(key+1) != '0':
                        has_label.append(str(key+1))
        if not EXCLUDE_GIRL.get() and not EXCLUDE_GIRL.get():
            if value['bg']=='red':
                value.config(bg='#C0C0C0')
                if str(key+1) not in all_label:
                    all_label.append(str(key+1))
