...
class_size=tk.Entry(root,textvariable=tk.StringVar(root,37),width=2,bd=0)
...
class button:
    def only():#被按钮调用抽一次的函数
        global random_num,label,p,all_label,id_,label_id_all,n,old,ago_place
        if int(class_size.get()) > int(old) and re_1.get():
            for b in range(int(old)+1,int(class_size.get())+1):
                all_label.append(str(b))
        if int(class_size.get()) < int(old) and re_1.get():
            for z in range(int(class_size.get())+1,int(old)+1):
                if str(z) in all_label:
                    all_label.remove(str(z))
        if all_label != []:
            old=str(class_size.get())
        if EXCLUDE_GIRL.get():
        ...
        if scale_change.get()>=2:
            ...
            if ro_change_place.get() and label_id_all != {} and ago_place != None and ago_place in label_id_all:
                label_id_all[ago_place].config(fg='black')
            ...
            if re_1.get() and label["text"] == "均已\n重复":
                ...
                if p-2 > -1 and not ro_change_place.get() and p-2 in label_id_all:
                    label_id_all[p-2].config(fg='black')
        else:
            for key,value in label_id_all.items():
                value.destroy()
            if label_id_all != {}:
...
def init_start():
    while only_button['text']=="截停":
        random_num=str(random.randint(1,37))
        label.config(text=random_num)
        label.update()

class add_del:  
    def __init__(self,root):
        self.slider = tk.Scale(root,from_=0, to=(int(class_size.get())-5)*30,length=160,showvalue=False,resolution=(int(class_size.get())-5),command=self.update_checkbox)
    ...
    def update_checkbox(self,_=None):  
        ...
        for i in range(int(class_size.get())):
            ...
            if  (20-slider_pos+i*30) <= 142 and (20-slider_pos+i*30) > 12:
...
class exclude:
    ...
    def girl():
        for G in range(int(first_girl.get()),int(class_size.get())+1):
            if EXCLUDE_GIRL.get() and G-1 in all_change:
                all_change[G-1].config(bg='red')
                if str(G) in all_label:
                    all_label.remove(str(G)) 
            elif not EXCLUDE_GIRL.get() and G-1 in all_change:
...
chronograph_show.place(x=90,y=400)
chronograph_init=tk.Entry(root,font=('黑体',14),textvariable=tk.StringVar(root,5),width=1,bd=0)
chronograph_init.place(x=165,y=432)
root.mainloop()
#我感觉我敲的比屎还臭
#再有下次一定写注释